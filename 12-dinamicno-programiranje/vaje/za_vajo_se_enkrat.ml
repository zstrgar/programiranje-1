...
let max_cheese cheese_matrix =
  let max_v = Array.length cheese_matrix in
  let max_s = Array.length cheese_matrix.(0) in
  let rec aux v s =
    if v >= max_v || s >= max_s then
      0
    else
      let desno = aux v (s + 1) in
      let dol = aux (v + 1) s in
      let trenutni_sir = cheese_matrix.(v).(s) in
      trenutni_sir + max desno dol
    in
    aux 0 0 

(*memoizacija*)

let max_cheese_memo cheese_matrix =
  let max_v = Array.length cheese_matrix in
  let max_s = Array.length cheese_matrix.(0) in
  let max_cheese' rekurzivna_max_cheese v s =
    if v >= max_v || s >= max_s then
      0
    else
      let desno = rekurzivna_max_cheese v (s + 1) in
      let dol = rekurzivna_max_cheese (v + 1) s in
      let trenutni_sir = cheese_matrix.(v).(s) in
      trenutni_sir + max desno dol
    in
  ...
  (*
  1. napišeš navadno ne za memoizirat
  2. znotrej namest tiste rekurzivne daš novo funkcijo, ki bo kot argument prejela uno rekurzivno
  3. primerno zamenjaš tm imena funkcij
  4. nakonc pokličeš memoiziraj_rec na tej odviti!
  *)
